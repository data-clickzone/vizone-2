#!/usr/bin/env python3
"""
Archive Grace Brands Meta creative assets to Google Drive.
...
DEFAULT_FOLDER_PREFIX = "Grace Brands Meta Creative Assets"
...
def graph_get(path_or_url: str, params: dict[str, Any] | None, token: str, version: str) -> dict[str, Any]:
    # 1 second delay between requests to relieve the API
    time.sleep(1)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Archive recent active Grace Brands Meta creative images to Google Drive."
    )
    ...
    parser.add_argument("--parent-folder-id", default=env_first("GRACE_BRANDS_DRIVE_PARENT_FOLDER_ID", "DRIVE_PARENT_FOLDER_ID"))
...
def main() -> int:
    ...
    if not token or not account_id:
        print("Missing GRACE_BRANDS_META_ACCESS_TOKEN and/or GRACE_BRANDS_META_AD_ACCOUNT_ID.", file=sys.stderr)
        return 2
    ...
    download_dir = Path(tempfile.mkdtemp(prefix="grace-brands-meta-assets-", dir=str(output_dir)))
    ...
    if service and folder_id:
        try:
            ...
            uploaded = upload_file(
                ...
                folder_id,
                manifest_path,
                "Grace Brands Meta creative asset archive manifest",
                app_properties={
                    "source": "meta_marketing_api",
